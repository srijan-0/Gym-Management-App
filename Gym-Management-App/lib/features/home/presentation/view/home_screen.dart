...
class _HomeScreenState extends State<HomeScreen>
    ...
    _accelerometerSubscription = accelerometerEvents.listen((event) {
      ...
      if (acceleration > _shakeThreshold) {
        if (lastShakeTime == null ||
            now.difference(lastShakeTime!) > _shakeCooldown) {
          shakeCount++;
          lastShakeTime = now;

          if (shakeCount >= 3) {
            if (now.difference(lastShakeTime!) <= _shakeWindow) {
              _handleLogout();
            }
            shakeCount = 0;
          }
      ...
      if (lastShakeTime != null &&
          now.difference(lastShakeTime!) > _shakeWindow) {
        shakeCount = 0;
      }
    });
  }
  ...
  Widget _buildHomePage(double screenWidth, double screenHeight) {
    bool isTablet = screenWidth > 600;
    double paddingHorizontal = isTablet ? 48 : 16;
    double paddingVertical = isTablet ? 24 : 16;

    double availableHeight =
        screenHeight - (isTablet ? 56 : 48) - (paddingVertical * 2);
    ...
    return isTablet
        ? Padding(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  flex: 1,
                  child: _buildStepProgress(isTablet, availableHeight * 0.9),
                ),
                SizedBox(width: 24),
                Expanded(
                  flex: 1,
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildStepProgress(isTablet, 180),
                SizedBox(height: 20),
                if (isTablet && screenWidth > 1000)
                  Column(
                    children: [
                      _buildMembershipAndBMICards(isTablet, 140),
                      SizedBox(height: 20),
                      _buildWaterBottle(height: 200, isTablet: isTablet),
                    ],
                  )
                else
                  Row(
  ...
  Widget _buildStepProgress(bool isTablet, double maxHeight) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          SizedBox(
            height: isTablet ? maxHeight * 0.6 : 180,
            width: isTablet ? maxHeight * 0.6 : 180,
            child: Stack(
              alignment: Alignment.center,
              children: [
                PieChart(
                  PieChartData(
                    ...
                    sectionsSpace: 2,
                    centerSpaceRadius: isTablet ? 100 : 70,
                  ),
                ),
                Column(
                  mainAxisAlignment: MainAxisAlignment.center,
